...
Iteration `20260321-140121` is now in progress: target one new `compliance` grounding for `shared-workbench-orchestration` so the next depth-balancing step stays inside an already-canonical low-risk collaboration-upkeep slice, with only the directly dependent pattern metadata and derived domain view updated alongside the new instance.
...
- Current refinement target: after support moved ahead, `compliance`, `research`, and `hr` are now the tied-thinnest domains. This iteration should use the clean `compliance` opening in `shared-workbench-orchestration`, because that already-canonical collaboration-upkeep pattern still trails its research, operations, support, and HR peers in domain depth while remaining tightly bounded.

## Ordered tasks

1. Ground `shared-workbench-orchestration` in `compliance` with one low-risk internal workbench-upkeep example that stays bounded at source refresh, note normalization, ownership preservation, and explicit hold-state management.
2. Update only the directly dependent canonical metadata and derived domain view so the new compliance collaboration slice is navigable without reopening broader family or vocabulary work.
...
- Current scoped follow-on: add one compliance-grounded `shared-workbench-orchestration` example, expand that canonical pattern to include `compliance`, and refresh the derived domain view so the collaboration-upkeep slice becomes explicit in the thinnest tied domain.
...
- Current scoped follow-on: once this compliance collaboration-upkeep slice lands, reassess whether `research` or `hr` remains the clearest next tied-thinnest domain target for another similarly bounded instance-plus-metadata refinement.
...
- Treat the current collaboration-family upkeep refinement as valid only if the primary output remains one internal compliance workbench artifact carried through source refresh, note normalization, ownership preservation, and explicit hold-state management rather than legal advice, disposition recommendation, regulator communication, or control execution.

## Expected outcome

This iteration should stay in depth-balancing mode: add one compliance-grounded collaboration-upkeep example plus directly dependent metadata/view updates in the already-canonical `shared-workbench-orchestration` area, and keep the work bounded to that one slice rather than opening a broader structural batch.
