...
7. After subagents finish, the orchestrator must:
   - review what changed
  - perform a short post-iteration reflection on workflow quality, prompt quality, ontology coherence, pattern quality, and loop/tooling friction
  - if that reflection surfaces a specific, durable, and dependency-aware improvement that is not already captured, add or update exactly one proposal under `.agent/proposals/`
  - keep proposals concrete and scoped to the loop system, prompt system, tooling, schema, vocabularies, views, patterns, ontology, governance, or evaluation
  - do not force a proposal every iteration; skip proposal authoring when there is no clearly justified improvement
...
- Treat post-run reflection as bounded process improvement, not an excuse for open-ended self-modification.
